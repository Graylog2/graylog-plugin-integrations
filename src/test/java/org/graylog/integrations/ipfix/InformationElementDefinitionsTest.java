...
public class InformationElementDefinitionsTest {
    ...
    public void buildPenToIedsMap() {
        ...
        try {
            ...
            FileUtils.writeStringToFile(tempFile, custDefStr);
            ObjectMapper objectMapper = new ObjectMapper();
            Map map = definitions.buildPenToIedsMap(objectMapper, java.util.Optional.of(tempFile));
            assertEquals(map.size(),2);

        } catch (IOException exp) {
            LOG.debug("Cannot process PCAP stream", exp);
        }
